...
export function CircularText({ icon: Icon, value, label, index }: CircularTextProps) {
  ...
  return (
    <div className="relative w-36 h-36 sm:w-48 sm:h-48 flex items-center justify-center group">
      {/* The rotating text */}
      <div
      ...
      <div className="text-center transition-transform duration-300 group-hover:scale-110">
        <Icon className="h-6 w-6 sm:h-10 sm:w-10 text-accent mx-auto mb-2 drop-shadow-[0_1px_2px_hsl(var(--accent)/0.7)]" />
        <p className="text-2xl sm:text-4xl font-headline text-white">{value}</p>
        <p className="text-xs text-neutral-300 mt-1 max-w-20 mx-auto">{label}</p>
      </div>
    </div>
  );
}
