...
import type {ScrollCallback} from '@studio-freight/lenis';
import {useLayoutEffect, useRef, useCallback, ReactNode} from 'react';
import { useIsMobile } from '@/hooks/use-mobile';

// A re-usable component for the scroll stack item
...
// The main scroll stack component
const ScrollStack = ({
  children,
  className = '',
  itemDistance = 100,
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const endElementSelector = '.scroll-stack-end';
    const endElement = useWindowScroll
      ? document.querySelector<HTMLElement>(endElementSelector)
      : scrollerRef.current?.querySelector<HTMLElement>(endElementSelector);
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      let blur = 0;
      if (effectiveBlurAmount > 0 && topCardIndex > -1 && i < topCardIndex) {
  ...
  const handleScroll = useCallback<ScrollCallback>(() => {
  ...
  useLayoutEffect(() => {
    ...
    const cards = Array.from(
      (useWindowScroll ? document.querySelectorAll(selector) : scroller?.querySelectorAll(selector)) as NodeListOf<HTMLDivElement>
    );
  ...
  }, [itemDistance, useWindowScroll, setupLenis, updateCardTransforms, children]);
  ...
  if (useWindowScroll) {
    return (
      <div className={containerClassName}>
        <div className="scroll-stack-inner pt-[20vh] px-4 sm:px-8 md:px-20 pb-[50rem] min-h-screen" style={{ transform: 'translateZ(0)' }}>
          {children}
  ...
  return (
    <div className={containerClassName} ref={scrollerRef} style={{...containerStyles, transform: 'translateZ(0)'}}>
